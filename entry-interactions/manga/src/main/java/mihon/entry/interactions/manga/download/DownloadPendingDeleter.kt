...
internal class DownloadPendingDeleter(
     ...
     */
    @Synchronized
    fun addChapters(chapters: List<EntryChapter>, manga: tachiyomi.domain.entry.model.Entry) {
     ...
     */
    @Synchronized
    fun getPendingChapters(): Map<tachiyomi.domain.entry.model.Entry, List<EntryChapter>> {
     ...
     */
    private fun List<ChapterEntry>.addUniqueById(chapters: List<EntryChapter>): List<ChapterEntry> {
     ...
     */
    private fun tachiyomi.domain.entry.model.Entry.toEntry() = MangaEntry(id, url, title, source)

    /**
     * Returns a chapter entry from a chapter model.
     */
    private fun EntryChapter.toEntry() = ChapterEntry(id, url, name, scanlator)

    /**
     * Returns a manga model from a manga entry.
     */
    private fun MangaEntry.toModel() = tachiyomi.domain.entry.model.Entry.create().copy(
        url = url,
        title = title,
        source = source,
        id = id,
     ...
     */
    private fun ChapterEntry.toModel() = EntryChapter.create().copy(
        id = id,
        url = url,
        name = name,
        scanlator = scanlator,
