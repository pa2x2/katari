...
The script creates or refreshes the Python environment, generates the Entry SDK API reference with Dokka, stages it with the Markdown documentation, and starts the local MkDocs server. Open <http://localhost:8000/> to review the complete site, including the API reference.

Normal `mkdocs serve` arguments can be passed to the script:

```bash
./scripts/serve-docs.sh --dev-addr 127.0.0.1:9000
```
...
8. Verify the generated API reference and strict documentation build.
