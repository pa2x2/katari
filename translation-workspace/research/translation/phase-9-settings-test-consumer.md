...
- engine selection is Automatic or an explicit known engine;
- target language is the effective app language or an explicit BCP-47 language.

The Settings UI does not create a device-wide Translation preference. Test source text, per-request source/target
...
- the shared UI supplies `TranslationSessionController` and `TranslationSessionHost`.
...
without source and target languages, so it is checked by `Test translation` and rendered through the existing typed
...
`Test translation` opens a transient, non-saveable text input dialog. Submission creates a normal
`TranslationRequest` using the saved engine/target selections and sends it through the production
`TranslationFeature` via the shared session controller.
