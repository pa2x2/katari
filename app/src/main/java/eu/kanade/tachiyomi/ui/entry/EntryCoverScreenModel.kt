...
class EntryCoverScreenModel(
...
) : StateScreenModel<Entry?>(null) {

    init {
        screenModelScope.launchIO {
            getEntry.subscribe(entryId)
                .collect { newEntry -> mutableState.update { newEntry } }
        }
    }
    ...
    /**
     * Save entry cover Bitmap to picture or temporary share directory.
     *
    ...
    private suspend fun saveCoverInternal(context: Context, temp: Boolean): Uri? {
        val entry = state.value ?: return null
    ...
    fun editCover(context: Context, data: Uri) {
        val entry = state.value ?: return
        screenModelScope.launchIO {
            context.contentResolver.openInputStream(data)?.use {
                try {
                    entry.editCover(Injekt.get(), it, coverCache = coverCache)
    ...
    fun deleteCustomCover(context: Context) {
        val entryId = state.value?.id ?: return
        screenModelScope.launchIO {
            try {
                coverCache.deleteCustomCover(entryId)
