...
| ----------------------- | ----------- | -------------------------------------------------------------------------------- |
| Katari app version      | `1.5.0`     | App release supplying the SDK runtime implementation                             |
| SDK artifact set        | `sdk-2.3.0` | Exact coordinated `entry-source-api` and `book-api` version used for compilation |
| Extension `versionName` | `2.3.8`     | Required API family `2.3`, extension revision `8`                                |
| Extension `versionCode` | `42`        | Android's monotonically increasing update number                                 |

The extension `versionName` is not the SDK SemVer. In this example, `2.3.8` does not mean the extension compiled against SDK `2.3.8`.
...
The Git tag and JitPack version use the `sdk-` prefix, such as `sdk-2.3.0`; the semantic version is `2.3.0`.
...
A minor SDK release creates another declared family. The Katari runtime supplying SDK 2.3 continues accepting extensions from families 2.0 through 2.2 because the minor releases are backward-compatible, while older Katari releases reject extensions requiring 2.3 APIs.
...
`sdk-2.3.0`, first supplied by Katari `1.5.0`, introduces dynamic filter suggestions. Katari accepts the `2.0`,
`2.1`, `2.2`, and `2.3` Entry SDK families.
